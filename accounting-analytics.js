...
function ensureUI(){
  ...
  card.innerHTML=`<div class="sectionHeader"><div><h2>会計済み実売上</h2><div class="muted">店舗会計を保存した分だけ集計</div></div><a href="./checkout.html" class="back" style="padding:7px 10px">会計画面</a></div><div class="metrics"><div class="metric"><div class="metricLabel">実売上</div><div id="actualSales" class="metricValue">¥0</div><div class="metricSub">会計済みのみ</div></div><div class="metric"><div class="metricLabel">会計件数</div><div id="actualCount" class="metricValue">0件</div><div class="metricSub">会計保存済み</div></div><div class="metric"><div class="metricLabel">現金</div><div id="actualCash" class="metricValue">¥0</div><div class="metricSub">現金会計の合計</div></div><div class="metric"><div class="metricLabel">クレジット</div><div id="actualCredit" class="metricValue">¥0</div><div class="metricSub">クレジット会計の合計</div></div><div class="metric"><div class="metricLabel">施術売上</div><div id="actualTreatment" class="metricValue">¥0</div><div class="metricSub">会計時の施術料金</div></div><div class="metric"><div class="metricLabel">追加メニュー売上</div><div id="actualRetail" class="metricValue">¥0</div><div class="metricSub">セラフィー・インパクト等</div></div></div><div id="actualPaymentUnknown" class="muted" style="margin-top:9px"></div><div id="actualAdjust" class="muted" style="margin-top:5px"></div><div id="actualProducts" style="margin-top:10px"></div><div id="actualStatus" class="status"></div>`;
...
async function load(){
  ...
  const {data:sales,error}=await supabase.from('nakano_sales').select('id,sale_date,treatment_amount,adjustment_amount,total_amount,payment_method').gte('sale_date',first).lt('sale_date',next).order('sale_date');
  ...
  const total=rows.reduce((s,x)=>s+(Number(x.total_amount)||0),0),treatment=rows.reduce((s,x)=>s+(Number(x.treatment_amount)||0),0),adjust=rows.reduce((s,x)=>s+(Number(x.adjustment_amount)||0),0),retail=items.reduce((s,x)=>s+(Number(x.amount)||0),0),cash=rows.filter(x=>x.payment_method==='cash').reduce((s,x)=>s+(Number(x.total_amount)||0),0),credit=rows.filter(x=>x.payment_method==='credit').reduce((s,x)=>s+(Number(x.total_amount)||0),0),unknown=rows.filter(x=>!x.payment_method).reduce((s,x)=>s+(Number(x.total_amount)||0),0);
  $('actualSales').textContent=yen(total);$('actualCount').textContent=`${rows.length}件`;$('actualCash').textContent=yen(cash);$('actualCredit').textContent=yen(credit);$('actualTreatment').textContent=yen(treatment);$('actualRetail').textContent=yen(retail);$('actualPaymentUnknown').textContent=unknown?`支払方法未設定 ${yen(unknown)}（過去会計など）`:'';$('actualAdjust').textContent=adjust?`その他調整額 合計 ${adjust>=0?'+':''}${yen(adjust)}`:'';
  ...
  $('actualProducts').innerHTML=products.length?`<div class="notice"><strong>追加メニュー内訳</strong><br>${products.map(x=>`${x.name} ${yen(x.price)} × ${x.qty} = ${yen(x.amount)}`).join('<br>')}</div>`:'';$('actualStatus').textContent='';
}
